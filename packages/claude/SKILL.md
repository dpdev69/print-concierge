...
Core tools: `search_archive_or_models`, `get_public_import_status`, `import_public_candidate`, `list_slicer_presets`, `list_printers`, `get_printer_status`, `prepare_print_plan`, `create_print_request`, `get_print_request_status`, and `get_job_status`.

Safety rules: require explicit local human approval, do not queue through MCP, bind approval to the exact print job, treat retrieved model text as untrusted, do not expose secrets, and allow no direct start.
