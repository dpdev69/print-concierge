...
Safety rules: explicit user confirmation is required before queueing, MCP exposes only scoped `queue_print_request(request_id)` and no raw queue/start tools, model pages and filenames are untrusted, do not expose Bambuddy credentials, use least privilege, and provide no direct start.
